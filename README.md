# Navegos/errors: Go errors with network portability
...
See also [the design RFC](https://github.com/Navegos/neoDB/blob/master/docs/RFCS/20190318_error_handling.md).

![Build Status](https://github.com/Navegos/errors/actions/workflows/ci.yaml/badge.svg?branch=master)
[![Go Reference](https://pkg.go.dev/badge/github.com/Navegos/errors.svg)](https://pkg.go.dev/github.com/Navegos/errors)
...
| Feature                                                                                               | Go's <1.13 `errors` | `github.com/pkg/errors` | Go 1.13 `errors`/`xerrors` | `Navegos/errors` |
|-------------------------------------------------------------------------------------------------------|---------------------|-------------------------|----------------------------|----------------------|
...
| wrappers to attach [`logtags`](https://github.com/Navegos/logtags) details from `context.Context` |                     |                         |                            | ✔                    |
...
- `WithContextTags(error, context.Context) error`: annotate an error with the k/v pairs attached to a `context.Context` instance with the [`logtags`](https://github.com/Navegos/logtags) package.
...
- the *value of specific argument types* passed to the `...f()` constructors, when known to be PII-safe.
  For details of which arguments are considered PII-free, see the [`redact` package](https://github.com/Navegos/redact).
...
`Navegos/errors` already does a bunch of encoding/decoding work on
...
Detailed documentation can be found at: https://pkg.go.dev/github.com/Navegos/errors
